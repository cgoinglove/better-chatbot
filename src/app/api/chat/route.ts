...
export async function POST(request: Request) {
  try {
    ...
    const payload = chatApiSchemaRequestBodySchema.parse(json);
    const authedSession = session!;

    return withRequestContext(
      {
        userId: authedSession.user.id,
        userEmail: authedSession.user.email,
        userName: authedSession.user.name,
...
async function handleChatRequest({
  ...
  payload: ChatApiSchemaRequestBody;
}) {
