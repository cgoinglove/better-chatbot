...
server.tool("file_write", "Overwrite or append content to a file.", {
    ...
    sudo: z.boolean().optional(),
}, async ({ file, content, append, leading_newline, trailing_newline, sudo, }) => {
...
server.tool("browser_view", "View content of the current browser page.", {}, async () => {
...
server.tool("github_list_repositories", "List all GitHub repositories mounted in the system.", {}, async () => {
    try {
        ...
        const getRepoDetails = (repoPath) => {
            try {
                ...
                let lastCommit = {};
                try {
                    const commitInfo = execSync('git log -1 --pretty=format:"%h|%s|%cr"', { cwd: repoPath, encoding: "utf8" }).trim();
        ...
        const scanLocalRepositories = () => {
            ...
            const uniqueRepos = allRepos.filter((repo, index, self) => index === self.findIndex((r) => r.path === repo.path));
            return uniqueRepos;
        };
        ...
        if (repositories.length === 0) {
            ...
            repositories.forEach((repo, index) => {
                ...
                if (repo.remote)
                    responseText += `   Remote: ${repo.remote}\n`;
                if (repo.branch)
                    responseText += `   Branch: ${repo.branch}\n`;
    ...
    try {
        ...
        try {
            ...
            if (hasGitHubRemote) {
                ...
                const remoteMatch = remotes.match(/origin\s+(https:\/\/github\.com\/[^\s]+|git@github\.com:[^\s]+)/);
        ...
        const response = await fetch("http://localhost:3000/api/github/add-local-repo", {
            method: "POST",
            headers: {
...
server.tool("github_pull_repository", "Pull the latest changes from the remote repository.", {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/github/${repository_id}`);
        if (!response.ok) {
            const errorText = await response.text();
            return {
                content: [
...
server.tool("github_index_repository", "Index or re-index a GitHub repository.", {
    ...
    try {
        ...
        const fetch = (await import("node-fetch")).default;
        const response = await fetch(`http://localhost:3000/api/github/${id}/index`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
    ...
    try {
        ...
        try {
            ...
            if (allBranches.length === 0) {
                ...
                const otherBranches = allBranches.filter((branch) => !branch.isCurrent);
        ...
        try {
            ...
            if (localBranches.includes(branch_name)) {
                ...
                const remoteBranches = remoteBranchesOutput
                    ...
                    .map((branch) => branch.trim())
                    .filter((branch) => branch)
                    .map((branch) => branch.startsWith("origin/") ? branch.substring(7) : branch);
...
server.tool("github_view_commit", "View details of a specific commit in a GitHub repository.", {
    ...
    try {
        ...
        try {
            // Get commit details
            const commitDetails = execSync(`git show --pretty=format:"%h|%an|%ae|%ad|%s" --date=iso ${commit_hash}`, {
                cwd: repo.path,
                encoding: "utf8",
                stdio: "pipe",
            });
            ...
            // Format the response
            const responseText = `Commit ${hash} in ${repo.name}:\n\n` +
                `Author: ${author} <${email}>\n` +
                `Date: ${date}\n` +
                `Message: ${message}\n\n` +
                `Changes:\n\n` +
...
server.tool("github_get_file", "Get the content of a file from a GitHub repository.", {
    ...
    try {
        ...
        const url = new URL(`http://localhost:3000/api/github/${repository_id}/file`);
...
server.tool("github_list_pull_requests", "List pull requests for a GitHub repository.", {
    ...
    try {
        ...
        const fetch = (await import("node-fetch")).default;
        // First, get the repository details
        const repoResponse = await fetch(`http://localhost:3000/api/github/${repository_id}`);
        if (!repoResponse.ok) {
        ...
        const accountResponse = await fetch(`http://localhost:3000/api/github/account`);
        ...
        const prResponse = await fetch(`https://api.github.com/repos/${owner}/${repoName}/pulls?state=${state}`, {
            headers: {
...
server.tool("github_view_pull_request", "View details of a specific pull request in a GitHub repository.", {
    ...
    try {
        ...
        const prResponse = await fetch(`https://api.github.com/repos/${owner}/${repoName}/pulls/${pull_number}`, {
            headers: {
                Authorization: `token ${account.accessToken}`,
                Accept: "application/vnd.github.v3+json",
            },
        ...
        // Fetch PR comments
        const commentsResponse = await fetch(`https://api.github.com/repos/${owner}/${repoName}/issues/${pull_number}/comments`, {
            headers: {
                Authorization: `token ${account.accessToken}`,
                Accept: "application/vnd.github.v3+json",
            },
...
server.tool("idle", "A special tool to indicate completion of all tasks.", {}, async () => {
