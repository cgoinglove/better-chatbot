...
exports.executeCodeSecurely = executeCodeSecurely;
...
let VENV_PYTHON = path.join(VENV_PATH, "Scripts", "python.exe");
...
const DIRECT_SANDBOX_PATH = path.resolve(
  process.cwd(),
  "src",
  "llm-sandbox",
  "direct_sandbox.py",
 ...
 */
async function executeCodeSecurely(code, language) {
...
`;
    ...
    let childProcess;
    try {
      childProcess = (0, child_process_1.spawn)(VENV_PYTHON, [tempScript], {
...
async function executeJavaScriptSandbox(scriptPath) {
  return new Promise((resolve) => {
    ...
    const wrapperScriptPath = path.join(
      os.tmpdir(),
      `js-sandbox-wrapper-${Date.now()}.js`,
    );
