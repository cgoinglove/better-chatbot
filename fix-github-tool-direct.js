// Direct fix for the GitHub repository listing tool
// This script directly modifies the custom MCP server file to fix the "require is not defined" error
// Run with: node --input-type=module fix-github-tool-direct.js
...
// Replace the implementation with a completely self-contained version
const fixedImplementation = `server.tool(
  ...
  async () => {
    try {
      // Self-contained implementation that doesn't use require()
      ...
      if (repositories.length === 0) {
        ...
        repositories.forEach((repo, index) => {
          ...
          if (repo.url) responseText += \`   Remote: \${repo.url}\\n\`;
...
writeFileSync(serverFilePath, fileContent, 'utf8');

console.log('Successfully fixed the github_list_repositories tool in the custom MCP server!');
console.log('The fix uses dynamic imports instead of require() to avoid the "require is not defined" error.');
console.log('You can now use the my-custom-server_github_list_repositories command to list your GitHub repositories.');

// Create a simple test script
const testScriptPath = join(__dirname, 'test-github-list-direct.js');
const testScriptContent = `// Simple script to test the GitHub repository listing functionality
// Run with: node --input-type=module test-github-list-direct.js
...
// Terminate the server after 15 seconds
setTimeout(() => {
...
console.log(`\nCreated a test script at ${testScriptPath}`);
console.log('You can run it with: node --input-type=module test-github-list-direct.js');
