...
// Run with: node fix-github-tool-custom.js

const fs = require('fs');
const path = require('path');

// Path to the custom MCP server file - using the correct path
...
if (!fs.existsSync(serverFilePath)) {
  ...
  console.error('Please provide the correct path to your custom MCP server file:');
  console.error('Usage: node fix-github-tool-custom.js <path-to-server-file>');
  process.exit(1);
}
...
if (!match) {
  ...
  console.error('The script is looking for a tool named "github_list_repositories" in your server file.');
  console.error('If your tool has a different name, please modify this script accordingly.');
  process.exit(1);
}
